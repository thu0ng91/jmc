 ...
 * @author     $Author: juny $
 * @version    $Id: template.php 344 2009-06-23 03:06:07Z juny $
 */
...
function truncate($str, $length=10, $trimmarker='', $html=1){
	...
	if($html && (strpos($str, '<') !== false || strpos($str, '&') !== false)){
		$length = $length - strlen($trimmarker); //要截取的字符串
...
class JieqiTpl{
	...
	/**
	 * 获得cache模式 0-不缓存 1-自动缓存 2-本次不缓存但是保存缓存 3-缓存变量数组，而不是html
	 * 
	 * @param      int         $num
	 * @access     public
	 * @return     void
	...
	/**
	 * 设置cache模式 0-不缓存 1-自动缓存 2-本次不缓存但是保存缓存 3-缓存变量数组，而不是html
	 * 
	 * @param      int         $num
	 * @access     public
	 * @return     void
	 ...
	 * @return     mixed
	 */
	function get_assign($vname){
		return $this->_tpl_vars[$vname];
	}
	...
	function fetch($resource_name, $cache_id = NULL, $compile_id = NULL, $cache_time = NULL, $over_time = NULL, $display = false){
		...
		}elseif ($this->caching == 3) {
			$_template_results=$this->is_cached($resource_name, $cache_id, $compile_id, $cache_time, $over_time, true);
			if (false !== $_template_results) {
				eval('$_temp_vars = '.trim($_template_results).';');
				if(is_array($_temp_vars)) foreach($_temp_vars as $k=>$v) if (!isset($this->_tpl_vars[$k])) $this->_tpl_vars[$k] = $v;
				//后面键值覆盖前面键值可能有问题
				//if(is_array($_temp_vars)) $this->_tpl_vars = array_merge($this->_tpl_vars, $_temp_vars);
				//eval('$this->_tpl_vars = '.trim($_template_results).';');
				unset($_temp_vars);
				$use_cachevars = true;
			}
		}
