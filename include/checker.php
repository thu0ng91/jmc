 ...
 * @author     $Author: juny $
 * @version    $Id: checker.php 344 2009-06-23 03:06:07Z juny $
 */
...
class JieqiChecker extends JieqiObject{
	 ...
	 * 检查禁止发帖的时间段
	 * 
	 * @param      string      $timeset 禁止发帖的时间段设置
	 * @access     public
	 * @return     bool 允许发帖返回true，禁止发帖返回false
	 */
	function deny_time($timeset){
		...
		foreach($times as $t){
			...
			if(($timefrom > $timeto && ($now >= $timefrom || $now < $timeto)) || ($timefrom < $timeto && $now >= $timefrom && $now < $timeto)) {
				return false;
			}
		}
		return true;
