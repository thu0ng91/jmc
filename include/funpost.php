 ...
 * @author     $Author: juny $
 * @version    $Id: funpost.php 344 2009-06-23 03:06:07Z juny $
 */
...
function jieqi_topic_vars($topic, $enableubb=true){
	...
	$ret['posttext'] = $topic->getVar('posttext', 'n');
	if($ret['posttext'] !== flase){
		if($enableubb){
			...
			$ret['posttext']=$jieqiTxtcvt->displayTarea($ret['posttext'], 0, 1, 1, 1, 1, 'screen.width*0.75');
		}else{
			if(!is_object($jieqiTxtcvt)){
				include_once(JIEQI_ROOT_PATH.'/lib/text/textconvert.php');
				$jieqiTxtcvt=TextConvert::getInstance('TextConvert');
...
function jieqi_post_checkpre($configs, &$check_errors){
	...
	if(!empty($jieqiConfigs['system']['postdenytimes'])){
		if(!$checker->deny_time($jieqiConfigs['system']['postdenytimes'])) $check_errors[] = sprintf($jieqiLang['system']['post_deny_times'], jieqi_htmlstr($jieqiConfigs['system']['postdenytimes']));
	}
	//检查时间间隔，是否允许发贴
...
function jieqi_post_attachdb(&$post_set, &$attachary, &$attachs_handler){
	foreach($attachary as $k=>$v){
		...
		if(isset($post_set['posttime'])) $newAttach->setVar('uptime', $post_set['posttime']);
