 ...
 * @author     $Author: juny $
 * @version    $Id: global.php 344 2009-06-23 03:06:07Z juny $
 */
...
if((!empty($_SERVER['SCRIPT_FILENAME']) && isset($_SERVER['argv']) && $_SERVER['SCRIPT_FILENAME'] == $_SERVER['argv'][0]) || (empty($_SERVER['SCRIPT_FILENAME']) && isset($_SERVER['argv']) && !empty($_SERVER['argv'][0]))) define('JIEQI_PHP_CLI', 1);
...
//在认证文件里面授权
define('JIEQI_VERSION_TYPE', 'Standard');
define('JIEQI_MODULE_VTYPE', 'Standard');
...
if(defined('JIEQI_CUSTOM_INCLUDE') && JIEQI_CUSTOM_INCLUDE == 1){
	...
	if( preg_match('/\.php$/i', $tmpstr)){
		...
		if(is_file($tmpstr) && preg_match('/^\w+\.inc\.php$/i', $tmpstr)) include_once($tmpstr);
	}
}
...
//程序开始时候的钩子
if(function_exists('jieqi_hooks_global')) jieqi_hooks_global();
...
<title>'.jieqi_htmlstr($title).'</title><link rel="stylesheet" type="text/css" media="all" href="'.JIEQI_URL.'/themes/'.JIEQI_THEME_NAME.'/style.css" /></head><body><div id="msgboard" style="position:absolute; left:210px; top:150px; width:350px; height:100px; z-index:1;"><table class="grid" width="100%" border="0" cellspacing="1" cellpadding="6" ><caption>'.jieqi_htmlstr($title).'</caption><tr><td class="even"><br />'.$content.'<br /><br />如不能自动跳转，<a href="'.$url.'">点击这里直接进入！</a><br /><br />程序设计：<a href="http://www.jieqi.com" target="_blank">杰奇网络</a><br /><br /></td></tr></table></div></body></html>';
			...
			//不显示跳转页面
			if(empty($title) && empty($content) && !headers_sent()){
				header('Location: '.$url);
			}else{
				echo $title.$content.'<script language="JavaScript" type="text/javascript">window.location=\''.$url.'\';</script>';
			}
		}
	}else{
...
function jieqi_useraction($action, &$params){
	...
	if(function_exists('jieqi_user_'.$action)) $fname($params);
}
	...
	if($addget){
		foreach($_GET as $k=>$v){
			if (!array_key_exists($k, $varary) && !in_array($k, $filter) && is_string($v)){
				if(!empty($charset_convert_out)) $v = $charset_convert_out($v);
