 ...
 * 生成静态网站首页
 *
 * refresh = 1参数用于强制更新静态页；
 * template 参数用于设置框架模板，默认是theme.html，设置了就在templates目录下找；
 * content 参数用于设置住内容模板，默认为空，设置了就在templates目录下找；
 * target 参数用于设置生成的文件名 默认index.html；
 * confirm = 1表示直接刷新，不提示确认信息；
 * 比如刷新静态首页的链接
 * 1、以下链接表示整个首页用 /templates/index.html 模板，区块配置用 /configs/blocks.php ，生成 /index.html
 * 用 http://www.domain.com/indexs.php?refresh=1&template=index.html&target=index.html&blocks=blocks 
 * 
 * 2、以下链接表示首页头尾框架用系统默认的theme.html模板，中间内容部分用 /templates/index.html 模板，区块配置用 /configs/blocks.php ，生成 /index.html
 * http://www.domain.com/indexs.php?refresh=1&content=index.html&target=index.html&blocks=blocks
 * 
 * 调用模板：无
 * 
 * @category   jieqicms
...
//生成的页面
if(empty($_REQUEST['target']) || strlen($_REQUEST['target']) > 30 || !preg_match('/^\w+\.html?$/', $_REQUEST['target'])) $_REQUEST['target']='index.html';
...
//包含区块参数
if(empty($_REQUEST['blocks']) || strlen($_REQUEST['blocks']) > 30 || !preg_match('/^\w+$/', $_REQUEST['blocks'])) $_REQUEST['blocks']='blocks';
jieqi_getconfigs('system', $_REQUEST['blocks'], 'jieqiBlocks');

//设置该页面的模板文件
if(!empty($_REQUEST['template']) && strlen($_REQUEST['template']) <= 30 && preg_match('/^\w+\.html?$/', $_REQUEST['template'])) $jieqiTset['jieqi_page_template']=JIEQI_ROOT_PATH.'/templates/'.trim($_REQUEST['template']);
...
//开始footer.php
//预处理主内容模板
if(!empty($_REQUEST['content']) && strlen($_REQUEST['content']) <= 30 && preg_match('/^\w+\.html?$/', $_REQUEST['content'])) $jieqiTset['jieqi_contents_template']=JIEQI_ROOT_PATH.'/templates/'.trim($_REQUEST['content']);
