 ...
 * @author     $Author: juny $
 * @version    $Id: getpass.php 344 2009-06-23 03:06:07Z juny $
 */
...
switch($_POST['action']) {
	...
	if(is_object($user)){
		if($user->getVar('email', 'n')==$_POST['email']){
			...
			$content = strpos($jieqiLang['system']['get_password_link'], '%s') ? sprintf($jieqiLang['system']['get_password_link'], JIEQI_USER_URL.'/setpass.php?id='.$user->getVar('uid').'&checkcode='.md5($user->getVar('pass'))) : $jieqiLang['system']['get_password_link'].JIEQI_URL.'/setpass.php?id='.$user->getVar('uid').'&checkcode='.md5($user->getVar('pass'));
