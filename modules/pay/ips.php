 ...
 * @author     $Author: juny $
 * @version    $Id: ips.php 272 2008-12-09 06:33:32Z juny $
 */
...
if(isset($_REQUEST['egold']) && is_numeric($_REQUEST['egold']) && $_REQUEST['egold']>0){
	...
	if(!$paylog_handler->insert($paylog)) jieqi_printfail($jieqiLang['pay']['add_paylog_error']);
		...
		if($_REQUEST['moneytype'] == 1){ 
			...
			$Currency=$jieqiLang['pay']['moneytype'];	
			$cardinfo=$jieqiLang['pay']['card_local'];			
			$Mer_code=$jieqiPayset[JIEQI_PAY_TYPE]['payid'];  //商户编号
			$Key=$jieqiPayset[JIEQI_PAY_TYPE]['paykey']; //密钥
			$Strcert=$jieqiPayset[JIEQI_PAY_TYPE]['strcert'];//证书号
			
		}
		$Currency=$jieqiPayset[JIEQI_PAY_TYPE]['moneytype'];
		...
		$Date=date("Ymd");//支付时间
		$Gateway_Type=$jieqiPayset[JIEQI_PAY_TYPE]['Gateway_Type'];
		...
		$SignMD5=md5($Billno.$Amount.$Date.$Currency.$Strcert);  //$OrderEncodeType=1情况
	//	echo $SignMD5;exit;
		...
		$jieqiTpl->assign('Gateway_Type', $Gateway_Type);
