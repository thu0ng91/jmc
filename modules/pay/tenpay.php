...
if(isset($_REQUEST['egold']) && is_numeric($_REQUEST['egold']) && $_REQUEST['egold']>0){
	...
	if(!$paylog_handler->insert($paylog)) jieqi_printfail($jieqiLang['pay']['add_paylog_error']);
		...
		$text="cmdno=".$cmdno."&date=".$date."&bargainor_id=".$bargainor_id."&transaction_id=".$transaction_id."&sp_billno=".$sp_billno."&total_fee=".$total_fee."&fee_type=".$fee_type."&return_url=".$return_url."&attach=".$attach."&spbill_create_ip=".$spbill_create_ip."&key=".$jieqiPayset[JIEQI_PAY_TYPE]['paykey'];
		...
		$jieqiTpl->assign('spbill_create_ip', $spbill_create_ip);
