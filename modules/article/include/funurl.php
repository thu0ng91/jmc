 ...
 * @return     string
 */
function jieqi_url_article_toplist($page=1, $sort='', $class=0){
	global $jieqiConfigs;
	global $article_dynamic_url;
	global $article_static_url;
	global $jieqiSort;
	...
	if(!empty($class)){
		...
		if(!isset($jieqiSort['article'][$class])) $class = 0;
	}
	...
	if(!empty($jieqiConfigs['article']['faketoplist'])){
		...
		$repfrom = array('<{$jieqi_url}>', '<{$sort}>', '<{$class}>');
		$repto = array(JIEQI_URL, $sort, $class);
		if(!empty($page)){
		...
		$ret = $article_dynamic_url.'/toplist.php?sort='.$sort;
