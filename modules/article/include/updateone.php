 ...
 * @author     $Author: juny $
 * @version    $Id: updateone.php 339 2009-06-23 03:03:24Z juny $
 */
		...
		if(!empty($errtext)){
			...
			if(!empty($article)){
				if($article->getVar('display') != 0){
					...
					//分类
					if(!empty($sort) && $sort != 'default' && isset($jieqiCollect['sortid'][$sort]) && intval($jieqiCollect['sortid'][$sort]) > 0 && $article->getVar('sortid', 'n') == intval($jieqiCollect['sortid']['default'])){
					...
					//全文标志
					if($fullarticle > 0 && $article->getVar('fullflag', 'n') < $fullarticle){
						$article->setVar('fullflag', $fullarticle);
						$uparticleinfo = true;
					}
					//更新文章信息
					if($uparticleinfo) $article_handler->insert($article);
