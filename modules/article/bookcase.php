 ...
 * @author     $Author: juny $
 * @version    $Id: bookcase.php 339 2009-06-23 03:03:24Z juny $
 */
...
if(!empty($_REQUEST['delid'])){
	...
	if(is_object($bookcase)){
		if($bookcase->getVar('userid')==$_SESSION['jieqiUserId']){
			...
			$article_handler->db->query('UPDATE '.jieqi_dbprefix('article_article').' SET goodnum=goodnum-1 WHERE articleid='.$bookcase->getVar('articleid', 'n').' AND goodnum>0');
