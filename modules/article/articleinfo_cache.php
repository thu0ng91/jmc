...
if(JIEQI_USE_CACHE) $jieqiTpl->setCaching(3);
else $jieqiTpl->setCaching(0);
if(!JIEQI_USE_CACHE || !$jieqiTpl->is_cached($jieqiTset['jieqi_contents_template'], $jieqiTset['jieqi_contents_cacheid'])){
	...
	elseif($article->getVar('display') != 0 && $jieqiUsersStatus != JIEQI_GROUP_ADMIN) jieqi_printfail($jieqiLang['article']['article_not_audit']);
		...
		if(isset($jieqiOption['article']['fullflag']['items'][$tmpvar])) $jieqiTpl->assign('fullflag', $jieqiOption['article']['fullflag']['items'][$tmpvar]);
		else $jieqiTpl->assign('fullflag', $jieqiOption['article']['fullflag']['items'][$jieqiOption['article']['fullflag']['default']]);
		
		$tmpvar = $article->getVar('permission');
		if(isset($jieqiOption['article']['permission']['items'][$tmpvar])) $jieqiTpl->assign('permission', $jieqiOption['article']['permission']['items'][$tmpvar]);
		else $jieqiTpl->assign('permission', $jieqiOption['article']['permission']['items'][$jieqiOption['article']['permission']['default']]);
		
		$tmpvar = $article->getVar('firstflag');
		if(isset($jieqiOption['article']['firstflag']['items'][$tmpvar])) $jieqiTpl->assign('firstflag', $jieqiOption['article']['firstflag']['items'][$tmpvar]);
		else $jieqiTpl->assign('firstflag', $jieqiOption['article']['firstflag']['items'][$jieqiOption['article']['firstflag']['default']]);
