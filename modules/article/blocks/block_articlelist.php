 ...
 * @author     $Author: juny $
 * @version    $Id: block_articlelist.php 339 2009-06-23 03:03:24Z juny $
 */
...
class BlockArticleArticlelist extends JieqiBlock
{
	...
	function BlockArticleArticlelist(&$vars){
		...
		if(!empty($this->blockvars['vars'])){
			...
			if($arynum>0){
				...
				if(in_array($varary[0], array('allvisit', 'monthvisit', 'weekvisit', 'dayvisit', 'allvote', 'monthvote', 'weekvote', 'dayvote', 'postdate', 'toptime', 'goodnum', 'size', 'lastupdate', 'lastvote', 'goodnew'))) $this->exevars['field']=$varary[0];
			}
			...
			if($arynum>2){
				...
				if(is_numeric($tmpvar)){
					...
					if(is_numeric(str_replace('|', '', $tmpvar1))) $this->exevars['sortid']=$tmpvar1;
	...
	function setContent($isreturn=false){
		...
		while($v = $article_handler->getObject($res)){
			...
			elseif($this->exevars['field']=='lastupdate' || $this->exevars['field']=='postdate' || $this->exevars['field']=='toptime' || $this->exevars['field']=='lastvote') $articlerows[$k]['visitnum']=date('m-d', $articlerows[$k]['visitnum']);
			$k++;
		}
