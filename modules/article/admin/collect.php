 ...
 * @author     $Author: juny $
 * @version    $Id: collect.php 339 2009-06-23 03:03:24Z juny $
 */
		...
		if(empty($errtext)) {
			...
			if(array_key_exists($_REQUEST['siteid'], $jieqiCollectsite) && $jieqiCollectsite[$_REQUEST['siteid']]['enable']=='1'){
				...
				if(is_object($articleobj)){
					...
					//分类
					if(!empty($sort) && $sort != 'default' && isset($jieqiCollect['sortid'][$sort]) && intval($jieqiCollect['sortid'][$sort]) > 0 && $articleobj->getVar('sortid', 'n') == intval($jieqiCollect['sortid']['default'])){
					...
					//全文标志
					if($fullarticle > 0 && $articleobj->getVar('fullflag', 'n') != $fullarticle){
						$articleobj->setVar('fullflag', $fullarticle);
						$uparticleinfo = true;
					}
					//更新文章信息
					if($uparticleinfo) $article_handler->insert($articleobj);
