 ...
 * @author     $Author: juny $
 * @version    $Id: search.php 339 2009-06-23 03:03:24Z juny $
 */
...
//搜索字段
if(!isset($_REQUEST['searchtype']) || ($_REQUEST['searchtype'] != 'author' && $_REQUEST['searchtype'] != 'keywords')) $_REQUEST['searchtype']='articlename';
if($_REQUEST['searchtype'] == 'author') $intstype=2;
elseif($_REQUEST['searchtype'] == 'keywords') $intstype=4;
else $intstype=1;
...
if($rescount == 1){
	...
	$jieqiTpl->assign('searchkey', $_REQUEST['searchkey']);
	$jieqiTpl->assign('searchtype', $_REQUEST['searchtype']);
	//处理页面跳转
