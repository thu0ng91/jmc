 ...
 * @author     $Author: juny $
 * @version    $Id: lastchapter.php 339 2009-06-23 03:03:24Z juny $
 */
...
if(file_exists(JIEQI_ROOT_PATH.'/files/obook/articlelink')){
	...
	if(file_exists($linkfile)){
		...
		if(!empty($jieqiObookdata['ochapter']['0']['ochapterid'])) $jumpurl=$obook_static_url.'/reader.php?aid='.intval($jieqiObookdata['obook']['obookid']).'&cid='.intval($jieqiObookdata['ochapter']['0']['ochapterid']);
		else $jumpurl=$obook_static_url.'/obookread.php?oid='.intval($jieqiObookdata['obook']['obookid']).'&page=index';
		$hasobook=1;
	}
}
