 ...
 * @author     $Author: juny $
 * @version    $Id: chapteredit.php 339 2009-06-23 03:03:24Z juny $
 */
...
if(!$canedit) jieqi_printfail(sprintf($jieqiLang['article']['noper_edit_chapter'], $typename));
//检查上传权限
...
switch($_REQUEST['action']){
	...
	default:
		...
		if(!empty($tmpvar)){
			...
			if(!is_array($attachrows)) $attachrows=array();
			$attachnum=count($attachrows);
			foreach($attachrows as $k=>$v) $attachrows[$k]['name'] = jieqi_htmlstr($attachrows[$k]['name']);
		}
