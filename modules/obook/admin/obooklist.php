...
if(isset($_REQUEST['action']) && !empty($_REQUEST['id'])){
	...
	switch($_REQUEST['action']){
		...
		case 'unsale':
			...
			break;
		case 'del':
			$obook_handler->execute('DELETE FROM '.jieqi_dbprefix('obook_obook').' WHERE obookid='.intval($_REQUEST['id']));
			$obook_handler->execute('SELECT ochapterid FROM '.jieqi_dbprefix('obook_ochapter').' WHERE obookid='.intval($_REQUEST['id']));
			$cids='';
			while($row = $obook_handler->getRow()){
				if(!empty($cids)) $cids .= ', ';
				$cids .= intval($row['ochapterid']);
			}
			$obook_handler->execute('DELETE FROM '.jieqi_dbprefix('obook_ochapter').' WHERE obookid='.intval($_REQUEST['id']));
			if(!empty($cids)) $obook_handler->execute('DELETE FROM '.jieqi_dbprefix('obook_ocontent').' WHERE ochapterid IN ('.$cids.')');
