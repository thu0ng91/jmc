	...
	function setContent($isreturn=false){
		...
		if(count($this->exevars) > 0){
			...
			while($v = $obook_handler->getObject()){
				...
				if($v->getVar('imgflag') > 0) $obookrows[$k]['url_image']=jieqi_uploadurl($jieqiConfigs['obook']['imagedir'], $jieqiConfigs['obook']['imageurl'], 'obook', $obook_static_url).jieqi_getsubdir($v->getVar('obookid')).'/'.$v->getVar('obookid').'/'.$v->getVar('obookid').'s'.$jieqiConfigs['obook']['imagetype'];
				else  $obookrows[$k]['url_image']=$obook_static_url.'/images/nocover.jpg';
				$k++;
			}
		}
